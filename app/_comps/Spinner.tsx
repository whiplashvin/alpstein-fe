function Spinner({ showPrice }: { showPrice: boolean }) {
  return (
    ...
    // ></span>
    <div
      className={`relative flex ${showPrice ? "size-7" : "size-4"} items-center justify-center overflow-hidden rounded-full bg-sky-400/20`}
    >
      <span className="absolute inset-0 h-full w-full animate-spin bg-conic from-sky-400 from-15% to-0% [animation-duration:0.7s]" />
      <div className="absolute top-1/2 left-1/2 z-20 h-[80%] w-[80%] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[var(--background)]"></div>
    </div>
  );
}
